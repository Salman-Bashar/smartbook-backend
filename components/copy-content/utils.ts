 ...
 *
 * Resource: https://github.com/portabletext/toolkit/blob/main/src/toPlainText.ts
 */
export function toPlainText(
  block: PortableTextBlock | PortableTextBlock[]
): string {
  const blocks = Array.isArray(block) ? block : [block];
  let text = "";

  blocks.forEach((current, index) => {
    if (!isPortableTextTextBlock(current)) {
      return;
    }
